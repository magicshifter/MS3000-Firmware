...
void loop()
{
  ...
  if (msGlobals.ggLastFrameMicros + msGlobals.ggSpeedMicros < micros())
  {
    ...
    msSystem.msLEDs.setPixels((xx + 0 * 3) & 0xF, 255, 0, 0, gs);

    msSystem.msLEDs.setPixels((xx + 1 * 3) & 0xF, 255, 255, 0, gs);
    msSystem.msLEDs.setPixels((xx + 2 * 3) & 0xF, 0, 255, 0, gs);

    msSystem.msLEDs.setPixels((xx + 3 * 3) & 0xF, 0, 255, 255, gs);
    msSystem.msLEDs.setPixels((xx + 4 * 3) & 0xF, 0, 0, 255, gs);
