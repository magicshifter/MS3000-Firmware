...
private:
  ...
  public:
    ...
    bool step()
    {
      ...
      msSystem.msLEDs.setPixels(lednr, 0, 255, 0, msGlobals.ggBrightness);

      for (int lC=0;lC<lednr;lC++) 
        msSystem.msLEDs.setPixels(lC, 0, 255, 0, msGlobals.ggBrightness); // !J! hack

      for (int lC=lednr+1;lC<MAX_LEDS;lC++) 
        msSystem.msLEDs.setPixels(lC, 255, 0, 0, msGlobals.ggBrightness); // !J! hack
