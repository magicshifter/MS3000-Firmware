	...
	// msSystem.slog("b:"); msSystem.slogln(String(b));
		...
		do
		{
			...
			do
			{
				...
				{
					msSystem.msLEDs.setLED(ledIdx, r, g, b, 255); // this could be more efficient memcopy the structure
				}
				else
				{
					msSystem.msLEDs.setLED(ledIdx, 0, 0, 0, 0); // this could be more efficient
				}
				bitMask <<= 1;
