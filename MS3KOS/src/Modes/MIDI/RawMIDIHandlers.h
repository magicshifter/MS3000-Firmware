...
{
	// raw start the _arp
	msMIDIArpeggiator._arp.arpStart();
}

void MIDI_Stop(miby_this_t a_miby)
{
	// raw stop the _arp
	msMIDIArpeggiator._arp.arpStop();
}

void MIDI_Program_Change(miby_this_t a_miby)
{
	msMIDIArpeggiator._arp.arpProgramChange(curr_midiview.midi_channel, MIBY_ARG0(a_miby));
}
...
void MIDI_Control_Change(miby_this_t a_miby)
{
	...
	if (MIBY_ARG0(a_miby) == 1) {
		msMIDIArpeggiator._arp.arp_bpm = LOWEST_ARP_TEMPO + (MIBY_ARG1(a_miby));
		msMIDIArpeggiator._arp.arp_beat_duration = ARP_DURATION_FOR_BPM(msMIDIArpeggiator._arp.arp_bpm);
	}
	// }
}
