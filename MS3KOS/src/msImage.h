...
void PlotBitmapColumn(const MSBitmap * bitmap, uint8_t frame,
					  uint8_t column, uint8_t startLed, byte * frameDest)
{
	uint8_t frameIdx = frame - bitmap->header.firstChar;
	if (frameIdx <= bitmap->header.maxFrame) {
		uint16_t absColumn =
			((uint16_t) frameIdx) *
			((uint16_t) bitmap->header.frameWidth) + column;
			  ...
			  uint8_t startLed, byte * frameDest)
{
	...
	PlotBitmapColumn(bitmap, ascii, column % bitmap->header.frameWidth,
					 startLed, frameDest);
}
			...
			for (int i = 0;; i++) {
				if (text[i] == '\n' || text[i] == 0) {
					if (i != lineStart) {
						...
						l_safeStrncpy(txtCollection[txtCount],
									  text + lineStart, len);
	...
	void getFrameData(int frameIdx, byte * frameDest) {
		...
		for (int i = 0; i < txtCount; i++) {
			...
			if (idx >= 0 && idx < txtSizes[i]) {
				PlotText(&txtFonts[i], txtCollection[i], idx,
						 txtPositions[i].y, frameDest);
			}
		}
	};
};
